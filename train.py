...
dataset_ratio = "50:25:25" # ratio of the dataset to use, separated by ':'
...
local_rank= int(os.environ.get('LOCAL_RANK', 0)) # for deepspeed, this is the local rank of the process
# -----------------------------------------------------------------------------
...
def create_deepspeed_config(
    train_batch_size=None,  # total batch size across all gpus
    ...
    config = {
        ...
        "scheduler": {
            ...
            "params": {
                ...
                "cos_min_ratio": min_lr / learning_rate,
                "total_num_steps": max_iters
            }
        },
...
ctx = nullcontext() # NOTE: disable autocast for deepspeed compatibility


# dataloader dict 
data_dir = dataset
dataloader_dict = dict()
for dataset_prefix_i in dataset_prefix.split(","):
    for datasubset_folder_i in [os.path.join(data_dir, f) for f in os.listdir(data_dir) if f.startswith(dataset_prefix_i)]:
        for split in ['train', 'val']:
            path_ = os.path.join(datasubset_folder_i, f'{split}.bin')
            dataloader_dict[path_] = DistributedDataLoader(filename=path_, B=1, T=block_size, process_rank=rank, num_processes=world_size)
...
#        output_dir=out_folder if master_process else '.',
    ...
    print(f"DeepSpeed initialized - no DDP wrapping needed")
...
if compile:
    ...
    if ref_model is not None and not ref_use_deepspeed:
        print("compiling the ref model... (takes a ~minute)")
    ...
    if ref_model_2 is not None and not ref_use_deepspeed:
