...
# download data by:
input_path = "Perper2024/synthetic-humans-1m-choice-"
output_path = "/prodcpfs/user/fengmingquan/dataset/processed-qwen2/phys-llm-31-qa"
fields = ["age", "location", "occupation", "wage","gender"]
...
if __name__ == '__main__':
    ...
    datasets = [load_dataset(input_path+field, num_proc=num_proc_load_dataset) for field in fields]
    train_datasets = [ds['train'].cast_column('answer_text', Value(dtype='string')) for ds in datasets]
    train_datasets = [ds.remove_columns(["choices1", "choices2", "choices3", "choices4"]) for ds in train_datasets]
    val_datasets = [ds['test'].cast_column('answer_text', Value(dtype='string')) for ds in datasets]
    val_datasets = [ds.remove_columns(["choices1", "choices2", "choices3", "choices4"]) for ds in val_datasets]
    merged_dataset = DatasetDict()
    merged_dataset['train'] = concatenate_datasets(train_datasets)
    merged_dataset['val'] = concatenate_datasets(val_datasets)
    ...
    def process(example):
        ...
        if output_path.endswith("qa"):
            text = f"Question: {example['question']}\nAnswer: {example['answer_text']}"
        else:
            raise ValueError("output_path should end with qa")
...
# nohup /cpfs/user/fengmingquan/miniconda3/envs/nanogpt/bin/python data/phys_llm_31/prepare_hf_qa.py > log/prepare_hf_11.log 2>&1 &


# processed-qwen2/phys-llm-31-bio/train.bin with 999500 samples and 90077477 tokens.  90 tokens per sample on average.
# processed-qwen2/phys-llm-31-story/train.bin with 999500 samples and 135249765 tokens.  135 tokens per sample on average.
# processed-qwen2/phys-llm-31-qa/train.bin   with 4950000 samples and 119521912 tokens.